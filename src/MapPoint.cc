...
//zoe 20181017
void MapPoint::ComputeDistinctiveDescriptorsRFNet()
{
    // Retrieve all observed descriptors
    ...
    for(size_t i=0;i<N;i++)
    {
        ...
        for(size_t j=i+1;j<N;j++)
        {
            float distij = ORBmatcher::DescriptorDistanceRFNet_float(vDescriptors[i],vDescriptors[j]);
            Distances[i][j]=distij;
            Distances[j][i]=distij;
        }
    }
...
std::vector<float> MapPoint::GetDescriptorRFNet()
{
    unique_lock<mutex> lock(mMutexFeatures);
