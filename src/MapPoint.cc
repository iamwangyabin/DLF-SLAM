...
//DLF
    void MapPoint::ComputeDistinctiveDescriptorsDLF() {
        // Retrieve all observed descriptors
        ...
        for (size_t i = 0; i < N; i++) {
            Distances[i][i] = 0;
            for (size_t j = i + 1; j < N; j++) {
                float distij = ORBmatcher::DescriptorDistance_float(vDescriptors[i], vDescriptors[j]);
                Distances[i][j] = distij;
                Distances[j][i] = distij;
            }
        }
    ...
    std::vector<float> MapPoint::GetDescriptorDLF() {
        unique_lock<mutex> lock(mMutexFeatures);
