...
//Copy Constructor
    Frame::Frame(const Frame &frame)
            : mpORBvocabulary(frame.mpORBvocabulary),
              ...
              mpORBextractorRight(frame.mpORBextractorRight),//zoe 20181016
              mTimeStamp(frame.mTimeStamp), mK(frame.mK.clone()), mDistCoef(frame.mDistCoef.clone()),
              mbf(frame.mbf), mb(frame.mb), mThDepth(frame.mThDepth), N(frame.N), mvKeys(frame.mvKeys),
              mvKpts(frame.mvKpts), mvKptsUn(frame.mvKptsUn),//zoe 20181016
              mvKeysRight(frame.mvKeysRight), mvKeysUn(frame.mvKeysUn), mvuRight(frame.mvuRight),
              mvDepth(frame.mvDepth), mBowVec(frame.mBowVec), mFeatVec(frame.mFeatVec),
              mDescriptors(frame.mDescriptors.clone()), mvDspts(frame.mvDspts),
              mDescriptorsRight(frame.mDescriptorsRight.clone()),// zoe 20181016
    ...
    //DLF
    void Frame::AssignFeaturesToGridDLF() {
        int nReserve = 0.5f * N / (FRAME_GRID_COLS * FRAME_GRID_ROWS);
        for (unsigned int i = 0; i < FRAME_GRID_COLS; i++)
            for (unsigned int j = 0; j < FRAME_GRID_ROWS; j++)
                mGrid[i][j].reserve(nReserve);
...
// zoe 待定?? 20181018 这个函数只在matcher文件里有出现
    ...
    // DLF
    void Frame::ComputeBoWDLF() {
        if (mBowVec.empty()) {
//            vector<cv::Mat> vCurrentDesc = Converter::toDescriptorVector(mDescriptors);
            mpDLFvocabulary->transform(mvDspts, mBowVec, mFeatVec, 4);
        }
    }
    ...
    // DLF
    void Frame::UndistortKeyPointsDLF() {
        if (mDistCoef.at<float>(0) == 0.0) {
            mvKptsUn = mvKpts;
            return;
        }
...
//zoe 这个函数不用改,用不到暂时 20181017
    ...
    void Frame::ComputeStereoFromRGBDDLF(const cv::Mat &imDepth) {
        mvuRight = vector<float>(N, -1);
        mvDepth = vector<float>(N, -1);
    ...
    cv::Mat Frame::UnprojectStereoDLF(const int &i) {
        const float z = mvDepth[i];
        if (z > 0) {
    ...
    // DLF SLAM
    Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, DLF *dlfhandler,
                 DLFVocabulary *vocDLF, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
            : mpDLFvocabulary(vocDLF), DLFextractor(dlfhandler),
        ...
        auto kps = DLFextractor->get_kps(img_float, patches);
        auto dess = DLFextractor->get_des(patches);

        const int kpts_num = 1000;//超参数,可以根据情况修改 每张照片提取特征点的个数 1000个,目前是跟ORB本身的个数是一样的.
        mvKpts.resize(kpts_num);//reserve 不行
        mvDspts.clear();

        torch::Tensor des_tensor_cpu = dess.cpu();
        ...
        if (mvKpts.empty())
            return;
        UndistortKeyPointsDLF();

        ComputeStereoFromRGBDDLF(imDepth);
