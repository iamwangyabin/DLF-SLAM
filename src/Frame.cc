...
Frame::Frame(const Frame &frame)
    :mpRFNETvocabulary(frame.mpRFNETvocabulary),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),//zoe 20181016
...
//zoe 20181016
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,RFNETVocabulary* vocrfnet, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
    :mpRFNETvocabulary(vocrfnet),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    // Frame ID
    mnId=nNextId++;
    ...
    string strRFNetPath = "/home/wang/workspace/data/rgbd_dataset_freiburg1_desk/RFNet_DBOW/";
    int kpts_num = 1000;//超参数,可以根据情况修改 每张照片提取特征点的个数 1000个,目前是跟ORB本身的个数是一样的.
    mnScaleLevels = 10; //超参数
    float Scale[mnScaleLevels] = {3.0,5.0,7.0,9.0,11.0,13.0,15.0,17.0,19.0,21.0};
	...
	kptsfile.open(strRFNetPath + strTimeStamp + "_kpts.txt");//打开文件
    featsfile.open(strRFNetPath + strTimeStamp + "_feats.txt");//打开文件
    kptsorifile.open(strRFNetPath + strTimeStamp + "_kpts_ori.txt");//打开文件
    kptsscalefile.open(strRFNetPath + strTimeStamp + "_kpts_scale.txt");//打开文件
    ...
    for (int i = 0; i < kpts_num; i++)//定义行循环
	{
        ...
        mvKpts[i].pt.x = fkpts[i][1];
        mvKpts[i].pt.y = fkpts[i][0];
    ...
    UndistortKeyPointsRFNet();
    
    ComputeStereoFromRGBDRFNet(imDepth);
...
// 20181015 zoe 增加
void Frame::AssignFeaturesToGridRFNet()
{
    int nReserve = 0.5f*N/(FRAME_GRID_COLS*FRAME_GRID_ROWS);
    for(unsigned int i=0; i<FRAME_GRID_COLS;i++)
        for (unsigned int j=0; j<FRAME_GRID_ROWS;j++)
...
//zoe 20181016
void Frame::ComputeBoWRFNet()
{
    if(mBowVec.empty())
    {
        //vector<cv::Mat> vCurrentDesc = Converter::toDescriptorVector(mDescriptors);
        mpRFNETvocabulary->transform(mvDspts,mBowVec,mFeatVec,4);
    }
}
...
// 20181215 zoe 新写的
void Frame::UndistortKeyPointsRFNet()
{
    if(mDistCoef.at<float>(0)==0.0)
    {
        mvKptsUn=mvKpts;
...
void Frame::ComputeStereoFromRGBDRFNet(const cv::Mat &imDepth)
{
    mvuRight = vector<float>(N,-1);
    mvDepth = vector<float>(N,-1);
...
// zoe20181016
cv::Mat Frame::UnprojectStereoRFNet(const int &i)
{
    const float z = mvDepth[i];
    if(z>0)
    {
