...
namespace ORB_SLAM2 {
    ...
    cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp) {
        ...
        // DLF
        mCurrentFrame = Frame(mImGray, imDepth, timestamp, DLFextraction, mpDLFVocabulary, mK, mDistCoef, mbf,
                              mThDepth);
...
//                    cv::Mat x3D = mCurrentFrame.UnprojectStereo(i);//zoe 20181016
                    cv::Mat x3D = mCurrentFrame.UnprojectStereoDLF(i);

                    MapPoint *pNewMP = new MapPoint(x3D, pKFini, mpMap);
                    pNewMP->AddObservation(pKFini, i);
                    pKFini->AddMapPoint(pNewMP, i);
                    //pNewMP->ComputeDistinctiveDescriptors();// zoe 20181016 20181017
                    pNewMP->ComputeDistinctiveDescriptorsDLF();
...
// zoe 20181017 单目初始化,暂时不用改
    void Tracking::MonocularInitialization() {
        ...
        if (!mpInitializer) {
            ...
            int nmatches = matcher.SearchForInitializationDLF(mInitialFrame, mCurrentFrame, mvbPrevMatched,
                                                                mvIniMatches, 100);
...
//        pKFcur->ComputeBoWLFNet();//zoe 20181017
        ...
        pKFini->ComputeBoWDLF();//DLF
        pKFcur->ComputeBoWDLF();//DLF
...
//        mCurrentFrame.ComputeBoWLFNet();
        ...
        int nmatches = matcher.SearchByBoWDLF(mpReferenceKF, mCurrentFrame, vpMapPointMatches);//zoe 20181017
    ...
    void Tracking::UpdateLastFrame() {
        ...
        for (size_t j = 0; j < vDepthIdx.size(); j++) {
            ...
            if (bCreateNew) {
                ...
                cv::Mat x3D = mLastFrame.UnprojectStereoDLF(i);
    ...
    bool Tracking::TrackWithMotionModel() {
        ...
        int nmatches = matcher.SearchByProjectionDLF(mCurrentFrame, mLastFrame, th, mSensor == System::MONOCULAR);

        // If few matches, uses a wider window search
        if (nmatches < 20) {
            fill(mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(), static_cast<MapPoint *>(NULL));
            //nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,2*th,mSensor==System::MONOCULAR);//zoe 20181017
            nmatches = matcher.SearchByProjectionDLF(mCurrentFrame, mLastFrame, 2 * th, mSensor == System::MONOCULAR);
        }
    ...
    bool Tracking::TrackLocalMap() {
        ...
        if (mnMatchesInliers < 20)// zoe 20181021 修改 DLF
            return false;
        else
            return true;
    }
    ...
    void Tracking::SearchLocalPoints() {
        ...
        if (nToMatch > 0) {
            ...
            matcher.SearchByProjectionDLF(mCurrentFrame, mvpLocalMapPoints, th);
        }
...
//        mCurrentFrame.ComputeBoWLFNet();
        ...
        for (int i = 0; i < nKFs; i++) {
            ...
            if (pKF->isBad())
                ...
                int nmatches = matcher.SearchByBoWDLF(pKF, mCurrentFrame, vvpMapPointMatches[i]);
            ...
            for (int i = 0; i < nKFs; i++) {
                ...
                if (!Tcw.empty()) {
                    ...
                    if (nGood < 50) {
                        ...
                        int nadditional = matcher2.SearchByProjectionDLF(mCurrentFrame, vpCandidateKFs[i], sFound, 10,
                                                                           FLOAT_MAX / 2);
                        ...
                        if (nadditional + nGood >= 50) {
                            ...
                            if (nGood > 30 && nGood < 50) {
                                ...
                                nadditional = matcher2.SearchByProjectionDLF(mCurrentFrame, vpCandidateKFs[i], sFound,
                                                                               3, FLOAT_MAX / 6);
    ...
    // DLF SLAM
    Tracking::Tracking(System *pSys, DLFVocabulary *pVocDLF, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer,
                       Map *pMap, KeyFrameDatabase *pKFDB, const string &strSettingPath, const int sensor)
            : mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpDLFVocabulary(pVocDLF),
              mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer *>(NULL)), mpSystem(pSys), mpViewer(NULL),
              mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0){

        DLFextraction = new DLF("/home/wang/workspace/RFNet_JIT/NASNet_0.1/des.pt","/home/wang/workspace/RFNet_JIT/NASNet_0.1/kpt.pt",32);
