...
namespace ORB_SLAM2 {
    ...
    KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB) :
            ...
            mvKpts(F.mvKpts), mvKptsUn(F.mvKptsUn),//zoe 20181016
            mvuRight(F.mvuRight), mvDepth(F.mvDepth), mDescriptors(F.mDescriptors.clone()),
            mvDspts(F.mvDspts),//zoe 20181019
            ...
            mpParent(NULL), mbNotErase(false),//zoe 20181016
            mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb / 2), mpMap(pMap),
            mpDLFvocabulary(F.mpDLFvocabulary)
            {
        mnId = nNextId++;
    ...
    void KeyFrame::ComputeBoWDLF() {
        if (mBowVec.empty() || mFeatVec.empty()) {
            //vector<cv::Mat> vCurrentDesc = Converter::toDescriptorVector(mDescriptors);
            // Feature vector associate features with nodes in the 4th level (from leaves up)
            // We assume the vocabulary tree has 6 levels, change the 4 otherwise
            mpDLFvocabulary->transform(mvDspts, mBowVec, mFeatVec, 4); //wang 4
        }
    }
...
//zoe mark 待修改
    ...
    // DLF
    cv::Mat KeyFrame::UnprojectStereoDLF(int i) {
        const float z = mvDepth[i];
        if (z > 0) {
