...
namespace ORB_SLAM2 {
    ...
    //DLF
    LoopClosing::LoopClosing(Map *pMap, KeyFrameDatabase *pDB, DLFVocabulary *pVocDLF, const bool bFixScale) :
            mbResetRequested(false), mbFinishRequested(false), mbFinished(true), mpMap(pMap),
            mpKeyFrameDB(pDB), mpDLFVocabulary(pVocDLF), mpMatchedKF(NULL), mLastLoopKFid(0), mbRunningGBA(false),
            mbFinishedGBA(true),
            mbStopGBA(false), mpThreadGBA(NULL), mbFixScale(bFixScale), mnFullBAIdx(0) {
        mnCovisibilityConsistencyTh = 3;
    }
    ...
    bool LoopClosing::DetectLoop() {
        ...
        for (size_t i = 0; i < vpConnectedKeyFrames.size(); i++) {
            ...
            float score = mpDLFVocabulary->score(CurrentBowVec, BowVec);
...
//            int nmatches = matcher.SearchByBoWLFNet(mpCurrentKF, pKF, vvpMapPointMatches[i]);
            int nmatches = matcher.SearchByBoWDLF(mpCurrentKF, pKF, vvpMapPointMatches[i]);
        ...
        while (nCandidates > 0 && !bMatch) {
            for (int i = 0; i < nInitialCandidates; i++) {
                ...
                if (!Scm.empty()) {
                    ...
                    matcher.SearchBySim3DLF(mpCurrentKF, pKF, vpMapPointMatches, s, R, t, 7.5);
        ...
        matcher.SearchByProjectionDLF(mpCurrentKF, mScw, mvpLoopMapPoints, mvpCurrentMatchedPoints, 10);
            ...
            for (size_t i = 0; i < mvpCurrentMatchedPoints.size(); i++) {
                if (mvpCurrentMatchedPoints[i]) {
                    ...
                    if (pCurMP)
                        ...
                        pLoopMP->ComputeDistinctiveDescriptorsDLF();
                    }
                }
            }
    ...
    void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap) {
        ...
        for (KeyFrameAndPose::const_iterator mit = CorrectedPosesMap.begin(), mend = CorrectedPosesMap.end();
            ...
            matcher.FuseDLF(pKF, cvScw, mvpLoopMapPoints, 4, vpReplacePoints);//zoe 20181019
